...
def fn_initialize_solution_easom():
    x = r.uniform(MIN_R_EASOM, MAX_R_EASOM)
    y = r.uniform(MIN_R_EASOM, MAX_R_EASOM)
    return x, y
...
#Main function by objective function
...
def fn_mainSA_easom():
    x, y = fn_initialize_solution_easom()
    energy = fn_easom_function(x,y)
    temperature = INITIAL_TEMPERATURE
    array_x = []
    array_y = []
    array_energy = []
    while(temperature > FINAL_TEMPERATURE): #termination criteria
        for i in range(ITERATIONS_EACH_TEMPERATURE):
            step_x, step_y = fn_exploration()
            new_energy = fn_easom_function(x+step_x, y+step_y)
            delta_energy = new_energy - energy
            if(delta_energy <= 0):
                x += step_x
                y += step_y
